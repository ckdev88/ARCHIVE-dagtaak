...
<template >
	<div :class="getWorkoutClassList(getCurrentDateInt(), item.datum)" :id="'workout-' + item.datum">
		<h3>
			<span class="datum">{{ item.datum }}</span><a :name="'workout-' + item.datum"></a>
		</h3>
		<div v-for="( i, key ) in  item.oefeningen ">
			<label :for="'workout-' + item.datum + '-' + key" style="display: block; cursor: pointer">
				<dt>{{ i.oefening }}</dt>
				<dd>
					<input type="number" :id="'workout-' + item.datum + '-' + key"
						:name="'workout-' + item.datum + '-' + key" onclick="this.select()" />&nbsp;
					...
					<span class="score">, score:
						...
						<span v-else>
							...
							<span v-else>{{ i.sets * i.reps }}</span> </span>, totaal: {{ i.gewicht }}
					</span>
					&nbsp;<sup class="badge" v-if="i.mutatie"><b>{{ i.mutatie }}</b></sup>
				</dd>
			</label>
		</div>
	</div>
